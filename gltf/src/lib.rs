...
impl MeshPrimitive {
    ...
    pub fn get_indices_data(&self, glb: &Glb) -> Vec<u32> {
        match self
            .indices
            .and_then(|accessor_index| glb.gltf.accessors.get(accessor_index))
        {
            Some(accessor) => {
            ...
            None => {
                let accessor_index = self
                    ...
                    .values()
                    .next()
                    .expect("No indices or attributes, mesh primitive is invalid");
                let accessor = glb.gltf.accessors.get(*accessor_index).unwrap();
                (0..accessor.count as u32).collect()
            }
        }
    }
}
